        ...
        <div class="container py-4">
            ...
            if ($action == 'list') {
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h2 class="text-azul fw-bold">Lista de Medicamentos</h2>
                    <a href="medicamento.php?action=create" class="btn btn-verde">Adicionar Novo Medicamento</a>
                </div>
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            ...
                            foreach ($medicamentos as $med) {
                                ...
                                echo "<td>" . htmlspecialchars($med['dosagem']) . "</td>";
                                echo "<td>" . htmlspecialchars($med['forma']) . "</td>";
                                echo "<td>" . htmlspecialchars($med['descricao']) . "</td>";
                                echo "<td>";
                                echo "<a href='medicamento.php?action=edit&id=" . $med['cod'] . "' class='btn btn-sm btn-primary me-2'>Editar</a>";
                ...
                $title = $action == 'create' ? 'Adicionar Novo Medicamento' : 'Editar Medicamento';
                ?>
                <h2 class="text-azul fw-bold mb-4"><?php echo $title; ?></h2>
                
                <form method="POST" action="">
                    <?php if ($action == 'edit') { ?>
                        <input type="hidden" name="cod" value="<?php echo htmlspecialchars($medicamento['cod']); ?>">
                    <?php } ?>
                    <div class="mb-3">
                        <label for="nome" class="form-label">Nome</label>
                        <input type="text" class="form-control" id="nome" name="nome" value="<?php echo $action == 'edit' ? htmlspecialchars($medicamento['nome']) : ''; ?>" required>
                    </div>
                    <div class="mb-3">
                        <label for="dosagem" class="form-label">Dosagem</label>
                        <input type="text" class="form-control" id="dosagem" name="dosagem" value="<?php echo $action == 'edit' ? htmlspecialchars($medicamento['dosagem']) : ''; ?>">
                    </div>
                    <div class="mb-3">
                        <label for="forma" class="form-label">Forma</label>
                        <input type="text" class="form-control" id="forma" name="forma" value="<?php echo $action == 'edit' ? htmlspecialchars($medicamento['forma']) : ''; ?>">
                    </div>
                    <div class="mb-3">
                        <label for="descricao" class="form-label">Descrição</label>
                        <textarea class="form-control" id="descricao" name="descricao"><?php echo $action == 'edit' ? htmlspecialchars($medicamento['descricao']) : ''; ?></textarea>
                    </div>
                    <button type="submit" class="btn btn-verde">Salvar</button>
                    <a href="medicamento.php" class="btn btn-secondary">Cancelar</a>
                </form>
                <?php
            }
            ?>
