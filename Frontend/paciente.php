...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    // DELETAR PACIENTE — apaga foto + prontuário junto
    if (isset($_POST['delete_id'])) {
        $pac = $pacController->getById($_POST['delete_id']);
        if ($pac) {
            // apaga foto do paciente
            apagarFotoPaciente($pac['foto'] ?? null);
            // apaga prontuários vinculados
        ...
        <div class="modal-content" style="border-radius:16px;overflow:hidden;">
            ...
            <div class="modal-header" style="background:linear-gradient(135deg,var(--teal-dark),var(--teal));border:none;padding:20px 24px;">
                <div class="d-flex align-items-center gap-3">
                    <div id="modalFotoWrap" class="modal-pac-foto-placeholder">
...
<?php foreach ($pacientes as $pac):
    ...
    // Foto vem direto do paciente
    $fotoSrc = !empty($pac['foto']) ? FOTO_PAC_URL . htmlspecialchars($pac['foto']) : '';
?>
    ...
    <div class="pac-card">
        ...
        <div class="pac-card-body">
            <div class="pac-info-row">
                ...
                <span><?= htmlspecialchars($pac['instituicao_nome'] ?? '—') ?></span>
            </div>
            <div class="pac-info-row">
                <i class="bi bi-envelope-fill"></i>
                <span style="word-break:break-all;"><?= htmlspecialchars($pac['email'] ?? '—') ?></span>
            </div>
            <div class="pac-info-row">
                <i class="bi bi-calendar-fill"></i>
                <span><?= htmlspecialchars($pac['data_nascimento'] ?? '—') ?></span>
            </div>
            <div class="pac-info-row">
                <i class="bi bi-droplet-fill"></i>
                <span>Sangue: <strong><?= htmlspecialchars($pront['tipo_sanguineo'] ?? '—') ?></strong></span>
            </div>
        </div>
...
<?php
/* ══════════════════════════════════════════════════════════
   FORMULÁRIO PACIENTE — criar / editar  (com foto)
══════════════════════════════════════════════════════════ */
elseif ($action == 'create' || $action == 'edit'): ?>

<div class="d-flex align-items-center gap-2 mb-4">
    <a href="paciente.php" style="color:#64748b;text-decoration:none;font-size:.9rem;">
    ...
    <form method="POST" enctype="multipart/form-data">
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <select name="fk_instituicao_cod" class="form-control" style="border-radius:8px;border:1px solid #e2e8f0;padding:10px 12px;">
                    ...
                    <option value="<?= $inst['cod'] ?>"
                        <?= ($action=='edit' && $paciente['fk_instituicao_cod']==$inst['cod']) ? 'selected' : '' ?>>
            ...
            <div class="col-md-6 mb-3">
                ...
                <input class="form-control" name="cpf"
                       value="<?= $action=='edit' ? htmlspecialchars($paciente['cpf']) : '' ?>"
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input class="form-control" name="nome"
                       value="<?= $action=='edit' ? htmlspecialchars($paciente['nome']) : '' ?>"
            ...
            <div class="col-md-6 mb-3">
                ...
                <input class="form-control" type="date" name="data_nascimento"
                       value="<?= $action=='edit' ? htmlspecialchars($paciente['data_nascimento']) : '' ?>"
                       style="border-radius:8px;border:1px solid #e2e8f0;padding:10px 12px;">
            </div>
        </div>
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <input class="form-control" name="email"
                       value="<?= $action=='edit' ? htmlspecialchars($paciente['email']) : '' ?>"
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                <label class="form-label" style="color:#475569;">
                    ...
                    <?php if ($action=='edit' && !empty($paciente['foto'])): ?>
                        <small class="text-muted">(selecione para substituir)</small>
                    <?php endif; ?>
                </label>

                <!-- Preview circular clicável -->
                <div class="foto-preview-wrap" id="fotoPacWrap"
                     onclick="document.getElementById('fotoFilePac').click()"
                ...
                <!-- input file — só envia no submit -->
                <input type="file" id="fotoFilePac" name="foto_file" accept="image/*"
                       class="form-control"
                       style="border-radius:8px;border:1px solid #e2e8f0;padding:10px 12px;">

                <small id="fotoStatusPac" class="text-muted d-block mt-1">
                    <?php if ($action=='edit' && !empty($paciente['foto'])): ?>
                        Foto atual: <?= htmlspecialchars($paciente['foto']) ?>
                    <?php else: ?>
            ...
            <div class="col-md-6 mb-3">
                ...
                <textarea class="form-control" name="endereco" rows="5"
                          placeholder="Rua, número, complemento..."
                          style="border-radius:8px;border:1px solid #e2e8f0;padding:10px 12px;"
                ><?= $action=='edit' ? htmlspecialchars($paciente['endereco']) : '' ?></textarea>
            </div>
        </div>

        <div class="d-flex gap-2 mt-2">
            <button type="submit" class="btn btn-verde"><i class="bi bi-check-lg me-1"></i> Salvar</button>
            <a href="paciente.php" class="btn btn-secondary" style="border-radius:8px;padding:10px 18px;">
...
<?php
/* ══════════════════════════════════════════════════════════
   FORMULÁRIO PRONTUÁRIO — criar / editar  (sem foto)
══════════════════════════════════════════════════════════ */
elseif ($action == 'prontuario_create' || $action == 'prontuario_edit'):
...
<div class="card-modern">
    ...
    <p class="text-muted mb-4">
        <?= $isEdit ? 'Atualize as informações médicas do paciente.' : 'Preencha as informações médicas do paciente.' ?>
    </p>

    <form method="POST">
        <?php if ($isEdit): ?>
            <input type="hidden" name="cod" value="<?= $prontuario['cod'] ?>">
        <?php endif; ?>
        ...
        <div class="row">
            <div class="col-md-6 mb-3">
                ...
                <select name="fk_paciente_cpf" class="form-control" style="border-radius:8px;border:1px solid #e2e8f0;padding:10px 12px;">
                    ...
                    <option value="<?= htmlspecialchars($p['cpf']) ?>"
                        <?= ($p['cpf'] == $cpfPresel) ? 'selected' : '' ?>>
            ...
            <div class="col-md-6 mb-3">
                ...
                <select name="tipo_sanguineo" class="form-control" style="border-radius:8px;border:1px solid #e2e8f0;padding:10px 12px;">
                    ...
                    <option value="<?= $bt ?>"
                        <?= ($isEdit && ($prontuario['tipo_sanguineo'] ?? '') == $bt) ? 'selected' : '' ?>>
                        <?= $bt ?>
                    </option>
                    <?php endforeach; ?>
                </select>
        ...
        <div class="row">
            <div class="col-12 mb-3">
                ...
                <textarea class="form-control" name="doencas_cronicas" rows="3"
                          placeholder="Ex: Diabetes, Hipertensão..."
                          style="border-radius:8px;border:1px solid #e2e8f0;padding:10px 12px;"
                ><?= $isEdit ? htmlspecialchars($prontuario['doencas_cronicas'] ?? '') : '' ?></textarea>
            </div>
            <div class="col-md-6 mb-3">
                <label class="form-label" style="color:#475569;">Doenças Genéticas</label>
                <textarea class="form-control" name="doencas_geneticas" rows="3"
                          placeholder="Ex: Hemofilia, Síndrome de Down..."
                          style="border-radius:8px;border:1px solid #e2e8f0;padding:10px 12px;"
                ><?= $isEdit ? htmlspecialchars($prontuario['doencas_geneticas'] ?? '') : '' ?></textarea>
            </div>
            <div class="col-md-6 mb-3">
                <label class="form-label" style="color:#475569;">Doenças Autoimunes</label>
                <textarea class="form-control" name="doencas_autoimunes" rows="3"
                          placeholder="Ex: Lúpus, Artrite reumatoide..."
                          style="border-radius:8px;border:1px solid #e2e8f0;padding:10px 12px;"
                ><?= $isEdit ? htmlspecialchars($prontuario['doencas_autoimunes'] ?? '') : '' ?></textarea>
            </div>
            <div class="col-12 mb-4">
                <label class="form-label" style="color:#475569;">Outros</label>
                <textarea class="form-control" name="outros" rows="2"
                          style="border-radius:8px;border:1px solid #e2e8f0;padding:10px 12px;"
                ><?= $isEdit ? htmlspecialchars($prontuario['outros'] ?? '') : '' ?></textarea>
            </div>
        </div>
...
<script>
// ── Abre modal com prontuário do paciente ─────────────────────────────────────
...
// ── Preview local foto do paciente (sem upload ainda) ─────────────────────────
