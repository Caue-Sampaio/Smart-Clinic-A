        ...
        <div class="container py-4">
            ...
            if ($action == 'list') {
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    <h2 class="text-azul fw-bold">Lista de Prescrever</h2>
                    <?php if (!isset($_SESSION['role']) || $_SESSION['role'] != 'paciente'): ?>
                    <a href="prescrever.php?action=create" class="btn btn-verde">Adicionar Novo Prescrever</a>
                    <?php endif; ?>
                </div>
                    ...
                    <table class="table table-striped table-hover">
                        ...
                        <tbody>
                            ...
                            foreach ($prescrivers as $pres) {
                                echo "<tr>";
                                echo "<td>" . htmlspecialchars($pres['fk_receita_cod']) . "</td>";
                                echo "<td>" . htmlspecialchars($pres['fk_medicamento_cod']) . "</td>";
                                echo "<td>" . htmlspecialchars($pres['descricao']) . "</td>";
                                echo "<td>" . htmlspecialchars($pres['modo_uso']) . "</td>";
                                echo "<td>";
                                if (!isset($_SESSION['role']) || $_SESSION['role'] != 'paciente') {
                                    echo "<a href='prescrever.php?action=edit&id1=" . $pres['fk_receita_cod'] . "&id2=" . $pres['fk_medicamento_cod'] . "' class='btn btn-sm btn-primary me-2'>Editar</a>";
                                    echo "<form method='POST' action='' style='display:inline;' onsubmit='return confirm(\"Tem certeza que deseja deletar?\")'>";
                                    echo "<input type='hidden' name='delete_id1' value='" . $pres['fk_receita_cod'] . "'>";
                                    echo "<input type='hidden' name='delete_id2' value='" . $pres['fk_medicamento_cod'] . "'>";
                ...
                $title = $action == 'create' ? 'Adicionar Novo Prescrever' : 'Editar Prescrever';
                ?>
                <h2 class="text-azul fw-bold mb-4"><?php echo $title; ?></h2>
                
                <form method="POST" action="">
                    <?php if ($action == 'edit') { ?>
                        <input type="hidden" name="fk_receita_cod" value="<?php echo htmlspecialchars($prescrever['fk_receita_cod']); ?>">
                        <input type="hidden" name="fk_medicamento_cod" value="<?php echo htmlspecialchars($prescrever['fk_medicamento_cod']); ?>">
                    <?php } ?>
                    <div class="mb-3">
                        <label for="fk_receita_cod" class="form-label">Código da Receita</label>
                        <input type="number" class="form-control" id="fk_receita_cod" name="fk_receita_cod" value="<?php echo $action == 'edit' ? htmlspecialchars($prescrever['fk_receita_cod']) : ''; ?>" required <?php echo $action == 'edit' ? 'readonly' : ''; ?>>
                    </div>
                    <div class="mb-3">
                        <label for="fk_medicamento_cod" class="form-label">Código do Medicamento</label>
                        <input type="number" class="form-control" id="fk_medicamento_cod" name="fk_medicamento_cod" value="<?php echo $action == 'edit' ? htmlspecialchars($prescrever['fk_medicamento_cod']) : ''; ?>" required <?php echo $action == 'edit' ? 'readonly' : ''; ?>>
                    </div>
                    <div class="mb-3">
                        <label for="descricao" class="form-label">Descrição</label>
                        <textarea class="form-control" id="descricao" name="descricao"><?php echo $action == 'edit' ? htmlspecialchars($prescrever['descricao']) : ''; ?></textarea>
                    </div>
                    <div class="mb-3">
                        <label for="modo_uso" class="form-label">Modo Uso</label>
                        <textarea class="form-control" id="modo_uso" name="modo_uso"><?php echo $action == 'edit' ? htmlspecialchars($prescrever['modo_uso']) : ''; ?></textarea>
                    </div>
                    <button type="submit" class="btn btn-verde">Salvar</button>
                    <a href="prescrever.php" class="btn btn-secondary">Cancelar</a>
                </form>
                <?php
            }
            ?>
